...
class Auth extends CI_Controller {
		...
		if (!empty($_POST)) {
			...
			if ($this->form_validation->run()) {
				if ($this->input->post('current_captcha') == $this->input->post('captcha')) {
					...
					$this->session->set_flashdata('SUCCESSMSGREGISTER', get_phrase('successfull_registration'));
					redirect('login');
				} else {
					$this->session->set_flashdata('ERROR', get_phrase('captcha_does_not_match'));

				}
			}
		}
		$this->load->view('front/auth/student_register');
	}
	...
	public function teacher_register() {
		...
		$this->load->view('front/auth/teacher_register');
	}
	...
	public function forgot_password() {
		if(!empty($_POST))
		{
				...
				if ($get->num_rows() > 0) 
				{
                    ...
                    $this->session->set_flashdata('SUCCESS', get_phrase('password_successfully_changed_please_check_your_email'));
                    redirect(base_url().'login' );

				}
			 	else 
			    {
					$this->session->set_flashdata('ERROR', get_phrase('email_id_does_not_exist'));
				}
		}


		$this->load->view('front/auth/forgot_password');
	}


	public function reset_password() {
		$this->load->view('front/auth/reset_password');
	}
