		...
		<div class="col-lg-5 student-form d-inline-block text-left">
			...
			<form method="POST" action="">
				...
				<div class="form-group">
					...
					<input type="text" name="email" value="<?=set_value('email')?>" class="form-control" placeholder="<?=get_phrase('enter_email')?>"  required="">
				</div>
				<div class="form-group">
					<label><?=get_phrase('password')?></label>
					<input type="password" class="form-control" placeholder="<?=get_phrase('enter_password')?>" name="password" required="">
				</div>

				<div class="form-group">
					<button class="btn" value="submit" type="submit">Login</button>
					<a style="color: #FF6D6D;display: block;margin-top: 21px;" href="<?=base_url()?>forgot-password"><?=get_phrase('forgot_password')?></a>
				</div>
